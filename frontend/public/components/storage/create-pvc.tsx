...
export class CreatePVCForm extends React.Component<CreatePVCFormProps, CreatePVCFormState> {
  state = {
    storageClass: '',
    storageClassAccessModes: ['ReadWriteOnce', 'ReadWriteMany', 'ReadOnlyMany'],
    pvcName: '',
  ...
  render() {
    const { accessMode, dropdownUnits, useSelector, nameValuePairs, requestSizeUnit, requestSizeValue, storageClassAccessModes } = this.state;
      ...
      <div>
        ...
        <div className="form-group">
          {this.state.accessModeRadios.map(radio => {
            let radioObj = null;
            const disabled = storageClassAccessModes.indexOf(radio.value) !== -1 ? false : true;

            storageClassAccessModes.forEach(mode => {
...
export type CreatePVCFormState = {
  storageClass: string;
  storageClassAccessModes: string[];
  pvcName: string;
  accessMode: string;
